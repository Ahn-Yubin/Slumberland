...
public class Collider {
	...
	public boolean isCollision(Collider col) {
		...
		for(Vector SA : separatingAxis) {
			double r1 = Math.abs(myRotatedNormal[0].dot(SA)) + Math.abs(myRotatedNormal[1].dot(SA));
			double r2 = Math.abs(othersRotatedNormal[0].dot(SA)) + Math.abs(othersRotatedNormal[1].dot(SA));
			double r0 = Math.abs((this.includedObject.getPosition().sub(col.includedObject.getPosition())).dot(SA));
			if(r0 > r1 + r2)
				return false;
		}
		return true;
	...
	public Vector[] getRotatedNormal(){
		...
		Vector rotatedNormalY = new Vector(-sin*this.getHeight()/2, cos*this.getHeight());
