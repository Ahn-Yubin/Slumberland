...
public class Controller implements KeyListener, MouseListener, Runnable{
	...
	public void run() { // Controller's Thread
		...
		try {
			while(true) {
				...
				for(Bullet b : model.getBulletList()) {
					if(b.getCollider().isCollision(model.getMonster().getCollider()))
						System.out.println("총알지나감");
					//System.out.println(b.getCollider().isCollision(model.getMonster().getCollider()));
				}
	...
	public void shoot() {
		...
		//-----------------------------------------------------------------------------------
		model.getBulletList().add(new Bullet(model.getPlayer().getPosition(), dv.mul(200).add(model.getPlayer().getSpeed())));
