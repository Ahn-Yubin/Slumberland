	...
	public void run() { // Controller's Thread
		...
		try {
			while(true) {
				...
				//model.getMonster().setDirection(model.getMonster().getDirection().add(model.getMonster().getDirection().normal().unit().mul(dt)));
				...
				while(itor.hasNext()) {
					if(itor.next().getCollider().isCollision(model.getMonster().getCollider())) {
							itor.remove();
					}
				}
				
				if(model.getPlayer().getCollider().isCollision(model.getMonster().getCollider())) {
					Vector p = model.getPlayer().getPosition().sub(model.getMonster().getPosition());
					Vector pUnit = model.getPlayer().getPosition().sub(model.getMonster().getPosition()).unit();
					Vector[] myRotatedNormal = model.getPlayer().getCollider().getRotatedNormal();
					Vector[] othersRotatedNormal = model.getMonster().getCollider().getRotatedNormal();
				
					double r1 = Math.abs(myRotatedNormal[0].dot(pUnit)) + Math.abs(myRotatedNormal[1].dot(pUnit));
					double r2 = Math.abs(othersRotatedNormal[0].dot(pUnit)) + Math.abs(othersRotatedNormal[1].dot(pUnit));
					double r0 = p.size();
					
					System.out.println("" + r1 + " " + r2 + " " + r0 + " " + p + " " + (r1+r2-r0) + " " + pUnit.mul(r1+r2-r0));
					model.getPlayer().addPosition(pUnit.mul(r1+r2-r0));
					model.getPlayer().addSpeed(model.getPlayer().getSpeed().mul(-1.3));
                }
