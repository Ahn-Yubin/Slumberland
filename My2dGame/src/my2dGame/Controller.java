		...
		try {
			while(true) {
				...
				if(model.getPlayer().getCollider().isCollision(model.getMonster().getCollider())) {
					...
					Vector v = model.getPlayer().getSpeed();
					double[] k = {0, 0, 0, 0};
					double r1 = 0, r2 = 0, r0 = 0, R = 0;
					
					double minimumR0 = 10000; // edit need
					double thatR = 10000; // edit need
					Vector thatAxis = new Vector();
					...
					for(Vector SA : separatingAxis) {
						...
						R = (model.getPlayer().getPosition().sub(model.getMonster().getPosition())).dot(SA);
						r0 = Math.abs(R);
					...
					double speedLossRate = 0.3;
					
					model.getPlayer().addPosition(thatAxis.mul(minimumR0));
					model.getPlayer().addSpeed(thatAxis.mul(- (2-speedLossRate) * v.dot(thatAxis)));
					// =================================================================================
				}
