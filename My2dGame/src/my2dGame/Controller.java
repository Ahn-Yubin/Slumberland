...
import physicalObject.entity.Boss;
	...
	public void run() { // Controller's Thread
		...
		try {
			while(true) {
				...
				model.getMap().setPosition(model.getInitialPlayerPosition().add(model.getPlayer().getPosition().sub(model.getInitialPlayerPosition()).mul(0.8)));
