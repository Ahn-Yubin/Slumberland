...
public class GameClient extends JFrame{
	...
	public GameClient() {
		...
		MenuButton startButton = new MenuButton(1225-280, 200, 280, 100, "게임 시작", font, 80.f, 90.f, Color.DARK_GRAY, Color.BLACK);
		startButton.addActionListener(new StartButtonClickListener());
		background.add(startButton);
		
		
		MenuButton howToPlayButton = new MenuButton(1225-280, 350, 280, 100, "게임 방법", font, 80.f, 90.f, Color.DARK_GRAY, Color.BLACK);
		background.add(howToPlayButton);
		
		MenuButton quitButton = new MenuButton(1225-100, 500, 100, 80, "종료", font, 60.f, 70.f, new Color(165, 49, 69), Color.DARK_GRAY);
...
class MenuButton extends JButton implements MouseListener{
	...
	public MenuButton(int x, int y, int width, int height, String text, Font font, float basicFontSize, float activatedFontSize, Color fontColor, Color borderColor) {
...
class BackGround extends JPanel{
	...
	public BackGround() {
		...
		try {
			this.background = ImageIO.read(new File("res/img/background.png"));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void paintComponent(Graphics g) {
		...
		//buffG.drawString("202117598 안유빈", 55, 500);
