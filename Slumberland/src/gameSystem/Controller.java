...
public class Controller implements KeyListener, MouseListener, Runnable{
	...
	public void run() { // Controller's Thread
		...
		try {
			while(true) {
				...
				view.getJetpackGaugeGUI().setAlpha(view.getDashCountGUI().getAlpha());
				
				if(model.getPlayer().getHp() == 0) {
					model.setGameState("GameOver");
					model.getPlayer().getSpeed().setX(0);
				}
				
				playerDirectionControll();

				model.getBackground().setPosition(model.getBackgroundInitialPosition().add(model.getPlayer().getPosition().sub(model.getBackgroundInitialPosition()).mul(0.8)));
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode() == KeyEvent.VK_R) {
			if(model.getGameState().equals("Running") && !model.getPlayer().getWeapon().isReloading()) {
				model.getPlayer().getWeapon().settingAmmo();
			}
			else if(model.getGameState().equals("GameOver")) {
				model.initializeFirstStage();
			}
		}
		if(e.getKeyCode() == KeyEvent.VK_ESCAPE && (model.getGameState().equals("GameOver") || model.getGameState().equals("Clear"))) {
			System.exit(0);
		}
		if(e.getKeyCode() == KeyEvent.VK_F1)
			view.setDeveloperMode(!view.isDeveloperMode());
	}
	...
	public void jetpack() { 
		...
		if (leftMouseClick && model.getPlayer().getHp() > 0) {
	...
	public void shoot() {
		if(model.getPlayer().getWeapon().getAmmo() > 0 && !model.getPlayer().getWeapon().isReloading() && model.getPlayer().getHp() > 0) {
	...
	public void dash() {
		...
		if(model.getPlayer().getDashCount() > 0 && model.getPlayer().getHp() > 0) {
	...
	public void setRightMouseClick(boolean rightMouseClick) {
